...
def normalize_landmarks(face_landmarks):
    ...
    scale = np.sqrt((nose_tip.x - forehead.x)**2 + (nose_tip.y - forehead.y)**2 + (nose_tip.z - forehead.z)**2)
...
def get_face_orientation(normalized_landmarks):
    ...
    vertical_upper_distance = nose_tip.y - forehead.y
    vertical_lower_distance = nose_tip.y - chin.y
    ...
    if vertical_upper_distance > 0.8 and vertical_upper_distance < 1 and vertical_lower_distance < -0.3 and vertical_lower_distance > -0.6:  # Adjust the vertical threshold as needed
        return 'Facing Down'
    elif vertical_upper_distance > 0.4 and vertical_upper_distance < 0.8 and vertical_lower_distance > -1.3 and vertical_lower_distance < -1.1:  # Adjust the vertical threshold as needed
        return 'Facing Up'
    ...
    while cap.isOpened():
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                cv2.putText(black_image, fps_text, (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
