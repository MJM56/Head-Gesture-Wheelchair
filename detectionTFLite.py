...
def Klasifikasi(Image, interpreter):
    input_details = interpreter.get_input_details()
    output_details = interpreter.get_output_details()
    
    img = copy.deepcopy(Image)
    img = cv2.resize(img, (200, 200))  # Resize according to your model's expected input
...
def PredictFaceMesh(NoKamera, LabelKelas):
    ...
    interpreter = tf.lite.Interpreter(model_path='D:/1 PraTA/Code/CNN/model.tflite')
        ...
        while cap.isOpened():
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    ...
                    idx = Klasifikasi(cropped_black, interpreter)
                    x = 50
                    y = 50
